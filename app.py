...
with shopee_tab:
    ...
    st.info(
        "Tempel link produk Shopee. AAE akan mencoba membaca data publik seperti nama, harga, "
        "rating, dan informasi produk. Komisi affiliate biasanya perlu diisi manual."
    )
        ...
        if not url.strip():
            ...
            if not result["ok"]:
                ...
                if result.get("fallback"):
                    st.warning("Gunakan tab 📊 Bandingkan Manual jika Shopee membatasi akses otomatis.")
            else:
                data = result["data"]
                st.success("Data produk berhasil dibaca. Periksa dan lengkapi data sebelum dinilai.")
                if data.get("image"):
...
st.divider()
st.caption("AAE MVP v4 — analisa link Shopee bersifat best-effort; halaman dinamis atau pembatasan akses dapat membuat sebagian data tidak terbaca.")
